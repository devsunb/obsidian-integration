...
export default class ConfluencePlugin extends Plugin {
  ...
  async publish(publishFilter?: string): Promise<void> {
    if (this.isSyncing) {
  ...
  override async onload() {
    ...
    this.addCommand({
      ...
      checkCallback: (checking: boolean) => {
        if (!checking) this.publish(this.activeLeafPath(this.workspace));
        return true;
      },
    });
